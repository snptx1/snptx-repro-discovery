<!-- GENERATED FILE. Do not edit by hand. Regenerate with `python pilot_phd/preprint/dl_forward/make_submission.py` after editing MANUSCRIPT_learned_representations_admet.md. -->
...
![Figure 0. System schematic. Molecular graphs feed the learned multi-task ensemble oracle; the random forest sits beside it as an explicit descriptor comparator; the calibrated decision engine (SPRT, conformal and selective prediction, abductive discovery) turns the oracle's uncertainty into a traceable go/no-go call, with every step logged to a DuckDB lineage store. (Regenerated by `architecture_figure_v2.py`.)](figures/fig0_architecture_v2.png)

**Figure 0.** System schematic. Molecular graphs feed the learned multi-task ensemble oracle; the random forest sits beside it as an explicit descriptor comparator; the calibrated decision engine (SPRT, conformal and selective prediction, abductive discovery) turns the oracle's uncertainty into a traceable go/no-go call, with every step logged to a DuckDB lineage store. Regenerated by `architecture_figure_v2.py`.

![Figure 1. Data-efficiency transfer curves. A shared multi-task graph encoder improves data-poor endpoints (hERG AUROC +0.008..+0.027, HIA +0.058..+0.071 at 25-50% data, Caco2 MAE -0.11..-0.17 at every fraction) over single-task graph baselines, without a raw-accuracy supremacy claim over the descriptor baseline. (Regenerated by `train_multitask_gnn.py, dl_forward/figures.py`.)](figures/fig_transfer_curves.png)

**Figure 1.** Data-efficiency transfer curves. A shared multi-task graph encoder improves data-poor endpoints (hERG AUROC +0.008..+0.027, HIA +0.058..+0.071 at 25-50% data, Caco2 MAE -0.11..-0.17 at every fraction) over single-task graph baselines, without a raw-accuracy supremacy claim over the descriptor baseline. Regenerated by `train_multitask_gnn.py, dl_forward/figures.py`.

![Figure 2. Self-supervised attribute-mask pretraining ablation. Classification deltas stay within seed noise; single-task regression (Solubility) is the one effect that clears it, MAE -0.126 +/- 0.053 at f=0.10. (Regenerated by `pretrain_ablation.py, dl_forward/figures.py`.)](figures/fig_pretrain_ablation.png)

**Figure 2.** Self-supervised attribute-mask pretraining ablation. Classification deltas stay within seed noise; single-task regression (Solubility) is the one effect that clears it, MAE -0.126 +/- 0.053 at f=0.10. Regenerated by `pretrain_ablation.py, dl_forward/figures.py`.

![Figure 3. Deep-ensemble calibration. The K=5 ensemble is the best-calibrated graph model (lowest NLL 0.482, Brier 0.153, AURC 0.126 among graph methods); the descriptor baseline remains strongest on raw ECE, which is reported rather than contested. (Regenerated by `make_ensemble_uncertainty.py, dl_forward/figures.py`.)](figures/fig_ensemble_calibration.png)

**Figure 3.** Deep-ensemble calibration. The K=5 ensemble is the best-calibrated graph model (lowest NLL 0.482, Brier 0.153, AURC 0.126 among graph methods); the descriptor baseline remains strongest on raw ECE, which is reported rather than contested. Regenerated by `make_ensemble_uncertainty.py, dl_forward/figures.py`.

![Figure 4. Conformal set efficiency and selective prediction under scaffold shift. The ensemble gives the sharpest 90%-coverage sets of any graph model and undercuts the descriptor baseline on AMES, hERG, and HIA. (Regenerated by `make_ensemble_uncertainty.py, dl_forward/figures.py`.)](figures/fig_conformal_efficiency.png)

**Figure 4.** Conformal set efficiency and selective prediction under scaffold shift. The ensemble gives the sharpest 90%-coverage sets of any graph model and undercuts the descriptor baseline on AMES, hERG, and HIA. Regenerated by `make_ensemble_uncertainty.py, dl_forward/figures.py`.

![Figure 5. Attention-attribution probe. Node-level attention does not recover the aromatic/basic-amine salience rule (AUROC ~0.5 on hERG and BBB), reported as a negative result and a limitation. (Regenerated by `attention_attribution.py, dl_forward/figures.py`.)](figures/fig_attention_probe.png)

**Figure 5.** Attention-attribution probe. Node-level attention does not recover the aromatic/basic-amine salience rule (AUROC ~0.5 on hERG and BBB), reported as a negative result and a limitation. Regenerated by `attention_attribution.py, dl_forward/figures.py`.

![Figure 6. End-to-end autonomous campaign with the learned oracle. SPRT returns GO on 3 of 4 classification endpoints and spends 150 versus a 213 fixed-sample budget, a 29.7% aggregate reduction; the harder cases (BBB weak shift, HIA undecided) are reported in full. (Regenerated by `e8_campaign_learned.py, dl_forward/figures.py`.)](figures/fig_campaign_efficiency.png)
